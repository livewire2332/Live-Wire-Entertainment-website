...
function playNew(i,type){
  ...
  else if(type==='pubcustomer'){
    let score=0,round=1,active=false,current=null;
    const customers=[
      ['🍺','The “I only came for one” customer','ONE DRINK WANDERER'],
      ['🎤','The customer who knows every lyric','PUB SINGER'],
      ['🎯','The darts expert who blames the board','DARTS LEGEND'],
      ['🕺','The one dancing before the music starts','EARLY DANCEFLOOR'],
      ['📱','The one filming absolutely everything','CONTENT CREATOR'],
      ['🍟','The one who orders food then steals everyone else’s','CHIP THIEF'],
      ['🪩','The disco enthusiast who never left the 70s','DISCO HERO'],
      ['🎧','The DJ who says “one more tune” for an hour','ONE MORE TUNE'],
      ['🚂','The Music Train passenger who missed the stop','TRAIN SPOTTER'],
      ['😂','The regular who knows everyone’s business','PUB NEWS DESK']
    ];
    o.innerHTML='<div class="lw-dart-score">🕵️ <strong>GUESS THAT PUB CUSTOMER</strong> · Score 0 · Round 1/5</div><div class="lw-dart-msg">Read the clues and identify the Live Wire pub character!</div><div class="lw-customer-scene"><div class="lw-customer-neon">LIVE WIRE<br><small>PUB FLOOR</small></div><div class="lw-customer-avatar">🍺</div><div class="lw-customer-name">WHO AM I?</div><div class="lw-customer-clues"><span>🔎 Loading clue...</span><span>🔎 Loading clue...</span><span>🔎 Loading clue...</span></div></div><div class="lw-customer-answers"></div><button type="button" class="lw-customer-reset">↻ NEW GAME</button>';
    c.innerHTML='';
    const scoreBox=o.querySelector('.lw-dart-score'),msg=o.querySelector('.lw-dart-msg'),avatar=o.querySelector('.lw-customer-avatar'),name=o.querySelector('.lw-customer-name'),clues=[...o.querySelectorAll('.lw-customer-clues span')],answers=o.querySelector('.lw-customer-answers'),reset=o.querySelector('.lw-customer-reset');
    const update=()=>{scoreBox.innerHTML='🕵️ <strong>GUESS THAT PUB CUSTOMER</strong> · Score '+score+' · Round '+round+'/5';};
    const next=()=>{
      if(round>5){answers.innerHTML='';msg.textContent='🏆 PUB FLOOR CLOSED! Final score: '+score+'/5';return;}
      current=customers[rand(customers.length)-1];avatar.textContent=current[0];name.textContent='WHO AM I?';clues[0].textContent='🔎 '+current[1];clues[1].textContent='🔎 Think of the '+current[2]+'!';clues[2].textContent='🔎 You have one guess...';
      answers.innerHTML='';
      const options=[current[2],'THE QUIET REGULAR','THE LOST TOURIST','THE SERIOUS CRITIC'].sort(()=>Math.random()-.5);
      options.forEach(opt=>{const b=document.createElement('button');b.type='button';b.textContent=opt;b.addEventListener('click',()=>decide(opt===current[2],b));answers.appendChild(b);});
      active=true;msg.textContent='🤔 Who is it? Pick your answer!';
    };
    const decide=(correct,b)=>{
      if(!active)return;active=false;answers.querySelectorAll('button').forEach(x=>x.disabled=true);b.classList.add(correct?'correct':'wrong');
      if(correct){score++;msg.textContent='🎉 CORRECT! You know your Live Wire locals!';}else{msg.textContent='😂 WRONG! The pub knows you don’t know your customers!';}
      update();round++;setTimeout(next,900);
    };
    reset.addEventListener('click',()=>{score=0;round=1;active=false;next();update();});
    next();update();
