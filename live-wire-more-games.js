...
function playNew(i,type){
  ...
  else if(type==='lastorders'){
    let score=0,round=1,active=false,timer=null;
    const rounds=[
      ['🎵','ONE MORE TUNE','The DJ has just played the perfect closing song. Do you let the night end?','🔔 CALL LAST ORDERS','🎶 ONE MORE TUNE'],
      ['🍻','THE TABLE ROUND','Someone shouts “LAST ORDERS!” and the whole table suddenly wants another round.','🔔 CLOSE THE BAR','🍻 ONE MORE ROUND'],
      ['🎤','KARAOKE EMERGENCY','A customer grabs the mic and promises this is absolutely their final song.','🔔 CLOSE THE BAR','🎤 ONE FINAL SONG'],
      ['💃','DANCEFLOOR STRAGGLER','The lights are coming up but one legendary dancer refuses to leave.','🔔 CLOSE THE BAR','🕺 ONE FINAL DANCE'],
      ['🍟','THE CHIP REQUEST','The kitchen is shutting. Someone asks for chips “very quickly”.','🔔 KITCHEN CLOSED','🍟 ONE LAST CHIPS']
    ];
    o.innerHTML='<div class="lw-dart-score">🔔 <strong>LAST ORDERS!</strong> · Score 0 · Round 1/5</div><div class="lw-dart-msg">The bar is closing... but the customers have other ideas!</div><div class="lw-lastorders-scene"><div class="lw-lastorders-sign">LAST<br>ORDERS</div><div class="lw-lastorders-bell">🔔</div><div class="lw-lastorders-customer">🍺</div><div class="lw-lastorders-counter">🍻 LIVE WIRE BAR 🍻</div></div><div class="lw-lastorders-actions"></div><div class="lw-lastorders-timer"></div><button type="button" class="lw-lastorders-reset">↻ NEW NIGHT</button>';
    c.innerHTML='';
    const scoreBox=o.querySelector('.lw-dart-score'),msg=o.querySelector('.lw-dart-msg'),scene=o.querySelector('.lw-lastorders-scene'),cust=o.querySelector('.lw-lastorders-customer'),actions=o.querySelector('.lw-lastorders-actions'),timerEl=o.querySelector('.lw-lastorders-timer'),reset=o.querySelector('.lw-lastorders-reset');
    const update=()=>scoreBox.innerHTML='🔔 <strong>LAST ORDERS!</strong> · Score '+score+' · Round '+round+'/5';
    const next=()=>{
      clearInterval(timer); timer=null;
      if(round>5){actions.innerHTML='';timerEl.textContent='';msg.textContent='🏁 BAR CLOSED! Final score: '+score+'/5';return;}
      const q=rounds[rand(rounds.length)-1];cust.textContent=q[0];scene.querySelector('.lw-lastorders-sign').innerHTML='LAST<br>ORDERS';msg.textContent=q[2];
      actions.innerHTML='';
      const opts=[q[3],q[4]];
      opts.forEach((txt,i)=>{const b=document.createElement('button');b.type='button';b.textContent=txt;b.addEventListener('click',()=>decide(i===0,b,q));actions.appendChild(b);});
      let t=8;timerEl.textContent='⏱️ '+t+' seconds';timer=setInterval(()=>{t--;timerEl.textContent='⏱️ '+t+' seconds';if(t<=0){clearInterval(timer);timer=null;decide(false,null,q,true);}},1000);
      active=true;update();
    };
    const decide=(close,b,q,timedOut=false)=>{
      if(!active)return;active=false;clearInterval(timer);timer=null;
      actions.querySelectorAll('button').forEach(x=>x.disabled=true);
      if(b)b.classList.add(close?'correct':'wrong');
      if(close&&!timedOut){score++;msg.textContent='🔔 LAST ORDERS CALLED! Sensible bar management!';}
      else if(timedOut){msg.textContent='⏰ TOO SLOW! The pub has closed itself!';}
      else{msg.textContent='😂 ONE MORE! The Live Wire family has spoken!';if(actions.children[1])actions.children[1].classList.add('wrong');}
      round++;update();setTimeout(next,900);
    };
    reset.addEventListener('click',()=>{score=0;round=1;active=false;clearInterval(timer);next();update();});
    next();update();
  }
